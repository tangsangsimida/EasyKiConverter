...
SymbolPin EasyedaImporter::importPinData(const QString &pinData)
{
    ...
    if (segments.size() >= 7) {
        ...
        if (settings.size() >= 8) { // 需要至少8个元素（包括"P"）
        ...
        // 段 5: dot (SymbolPinDotBis)
        if (segments[5].size() >= 3) {
            pin.dot.isDisplayed = (segments[5][0] == "show");
            pin.dot.circleX = segments[5][1].toDouble();
            pin.dot.circleY = segments[5][2].toDouble();
        }
...
QList<QStringList> EasyedaImporter::parsePinDataString(const QString &pinData) const
{
    ...
    QStringList segments = pinData.split("^^");
