...
QList<QPointF> SvgPathParser::parsePath(const QString &path)
{
    ...
    while (i < tokens.size()) {
        ...
        // 处理A/a（Arc）命令
            ...
            double rx = tokens[i].toDouble(&okRx); i++;
            double ry = tokens[i].toDouble(&okRy); i++;
            double xRotation = tokens[i].toDouble(&okXRot); i++;
            int largeArcFlag = tokens[i].toInt(&okLarge); i++;
            int sweepFlag = tokens[i].toInt(&okSweep); i++;
            double endX = tokens[i].toDouble(&okX); i++;
            double endY = tokens[i].toDouble(&okY); i++;
                                        ...
                                        bool largeArcFlag, bool sweepFlag, const QPointF &endPoint)
{
    ...
    // 规范化角度
    while (startAngle < 0) startAngle += 2 * PI;
...
{
    QList<QPointF> points;
    const int splitCount = 32; // 分割成32段
