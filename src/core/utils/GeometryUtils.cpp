...
QList<GeometryUtils::SvgPoint> GeometryUtils::parseSvgPath(const QString& path) {
    ...
    for (int i = 0; i < tokens.size();) {
        ...
        if (token == "M" || token == "m") {
            ...
            QList<SvgPoint> bezierPoints = bezierToPolyline(currentX, currentY, cp1x, cp1y, cp2x, cp2y, x, y);
...
QList<GeometryUtils::SvgPoint> GeometryUtils::bezierToPolyline(double startX,
                                                               double startY,
                                                               double cp1X,
                                                               double cp1Y,
                                                               double cp2X,
