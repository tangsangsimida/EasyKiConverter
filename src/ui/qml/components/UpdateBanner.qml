...
Rectangle {
    ...
    RowLayout {
        ...
        ColumnLayout {
            ...
            Text {
                text: updateChecker && updateChecker.releaseName && updateChecker.releaseName.length > 0
                    ? qsTranslate("MainWindow", "当前版本 %1，最新发布：%2").arg(updateChecker.currentVersion).arg(updateChecker.releaseName)
                    : qsTranslate("MainWindow", "当前版本 %1，可前往 GitHub 查看发布说明。").arg(updateChecker ? updateChecker.currentVersion : "")
