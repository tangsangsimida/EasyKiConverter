...
namespace EasyKiConverter
{
    ...
    void MainController::processNextComponent()
    {
        if (!m_isExporting || m_currentComponentIndex >= m_componentList.count())
        {
            ...
            if (m_exportSymbol && !m_allSymbols.isEmpty())
            {
                ...
                if (m_exporterSymbol->exportSymbolLibrary(newSymbols, m_libName, symbolFilePath, true))
                { // true = 追加模式
        ...
        if (m_exportFootprint)
        {
            ...
            for (const auto &data : m_collectedComponents)
            {
                ...
                if (m_exportModel3D && !data.footprintData.model3D().uuid().isEmpty())
                {
                    ...
                    // 将 stepData 设置到 footprintData 的 Model3DData 中
                    // 这样 generateModel3D 函数就能正确检测到 STEP 数据并导出 STEP 模型参数
                    if (!data.stepData.isEmpty() && data.stepData.size() > 100)
                    {
                        qDebug() << "Setting STEP data to Model3DData for component:" << data.componentId
                                 << "size:" << data.stepData.size();
                        FootprintData modifiedFootprintData = data.footprintData;
                        Model3DData model3D = modifiedFootprintData.model3D();
                        model3D.setStep(data.stepData);
                        modifiedFootprintData.setModel3D(model3D);
                        if (!m_exporterFootprint->exportFootprint(modifiedFootprintData, footprintPath, model3DPath))
                        {
    ...
    ComponentExportTask::ComponentExportTask(
        ...
        QObject *parent)
        : QObject(parent), QRunnable(), m_componentId(componentId), m_outputPath(outputPath), m_libName(libName), m_exportSymbol(exportSymbol), m_exportFootprint(exportFootprint), m_exportModel3D(exportModel3D), m_controller(controller)
    {
        setAutoDelete(true);
    }
        ...
        try
        {
            ...
            if (m_exportModel3D && !footprintData.model3D().uuid().isEmpty())
            {
                ...
                for (int attempt = 0; attempt < maxStepRetries; ++attempt)
                {
                    ...
                    if (uuid == modelUuid) {
                        qDebug() << "UUID match! STEP data size:" << data.size();
                        if (data.size() > 100) {
                            qDebug() << "STEP data is valid (> 100 bytes), first 100 chars:" << QString::fromLatin1(data.left(100));
                            attemptData = data;
                            attemptSuccess = true;
                        } else {
                            qWarning() << "STEP data is too small (" << data.size() << " bytes), treating as failure";
                            attemptSuccess = false;
                            attemptError = QString("STEP data too small (%1 bytes)").arg(data.size());
                        }
                        stepLoop.quit();
                    } });
                    ...
                    if (attemptSuccess)
                    {
                        stepData = attemptData;
                        stepSuccess = true;
                        qDebug() << "STEP data downloaded successfully on attempt" << (attempt + 1) << ", size:" << stepData.size();
                        break;
                    }
                    else
                    {
                        qWarning() << "STEP download attempt" << (attempt + 1) << "failed:" << attemptError;

                        // 如果不是最后一次尝试，使用指数退避等待后重试
                        if (attempt < maxStepRetries - 1)
                        {
                            int delayMs = static_cast<int>(std::pow(2, attempt) * 1000); // 1s, 2s, 4s
                            qDebug() << "Waiting" << delayMs << "ms before retry (exponential backoff)";
                ...
                if (!stepSuccess)
                {
                    qWarning() << "Failed to download STEP data for:" << m_componentId << "after" << maxStepRetries << "attempts";
                }
                else
                {
