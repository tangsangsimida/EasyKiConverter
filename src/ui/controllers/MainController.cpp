...
namespace EasyKiConverter
{
    ...
    void MainController::startExport()
    {
        ...
        if (m_componentList.count() > 1 && m_useParallelExport)
        {
            ...
            // TODO: 更新为使用新的 ComponentExportTask 接口
            // 临时注释：新的 ComponentExportTask 需要 ComponentData 和 ExportOptions
            // 需要先收集数据，然后再创建任务
            /*
            // 为每个元件创建并启动数据收集任务
            for (const QString &componentId : m_componentList)
            {
                // 使用新的 ComponentExportTask 构造函数
            ...
            */
            ...
            // 暂时使用串行导出
            qDebug() << "Parallel export temporarily disabled, using serial export";
