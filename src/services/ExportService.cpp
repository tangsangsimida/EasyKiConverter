...
void ExportService::executeExportPipelineWithData(const QList<ComponentData>& componentDataList,
            ...
            for (auto& model : allModels) {
                ...
                for (const auto& exportData : m_exportDataList) {
                    if (exportData.model3DData.uuid() == model.uuid() &&
                        !exportData.footprintData.info().name.isEmpty()) {
...
bool ExportService::exportSymbolLibrary(const QList<SymbolData>& symbols,
...
bool ExportService::exportFootprintLibrary(const QList<FootprintData>& footprints,
                                           const QString& libName,
                                           const QString& filePath) {
...
void ExportService::executeExportPipelineWithDataParallel(const QList<ComponentData>& componentDataList,
                                                          const ExportOptions& options) {
