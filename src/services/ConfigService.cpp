...
/** @brief 开始批量配置更新，暂缓重复写入配置文件。 */
void ConfigService::beginBatchUpdate() {
    QMutexLocker locker(&m_configMutex);
    m_batchUpdateDepth++;
}

/** @brief 结束批量配置更新，并在需要时提交一次保存。 */
