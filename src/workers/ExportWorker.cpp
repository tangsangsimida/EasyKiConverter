...
ExportWorker::ExportWorker(const QString& componentId,
...
void ExportWorker::run() {
    ...
    try {
        ...
        if (m_exportModel3D && m_footprintData && !m_footprintData->model3D().uuid().isEmpty()) {
            ...
            // 计算3D模型导出路径
            model3DPath = QString("%1/%2.3dshapes/%3.wrl").arg(m_outputPath, m_libName, sanitizedName);
        }
...
bool ExportWorker::exportSymbolLibrary() {
    try {
        ...
        bool success = exporterSymbol.exportSymbolLibrary(symbolList,
                                                          m_libName,
                                                          symbolFilePath,
                                                          true  // appendMode
        );
...
bool ExportWorker::exportFootprintLibrary(const QString& model3DPath) {
    try {
        QString footprintFilePath =
            QString("%1/%2.pretty/%3.kicad_mod").arg(m_outputPath, m_libName, m_footprintData->info().name);

        ExporterFootprint exporterFootprint;

        bool success = exporterFootprint.exportFootprint(*m_footprintData, footprintFilePath, model3DPath);
