...
bool PathSecurity::isValidPathComponent(const QString& name) {
    ...
    if (name.contains('/') || name.contains(QChar(0x5C))) return false;
    // 检查是否包含路径遍历
...
bool PathSecurity::isSafePath(const QString& fullPath, const QString& baseDir) {
    ...
    QString absTarget = targetInfo.exists() ? targetInfo.canonicalFilePath() : QDir::cleanPath(targetInfo.absoluteFilePath());
    ...
    while (targetNoSlash.endsWith('/')) targetNoSlash.chop(1);
...
bool PathSecurity::safeRemoveRecursively(const QString& path, int maxFileCount) {
    ...
    if (count > maxFileCount) {
        qCritical() << "Security Alert: Recursive delete aborted. File count" << count 
                    << "exceeds limit of" << maxFileCount << "in path:" << path;
        return false;
    }
...
int PathSecurity::countFilesRecursively(const QString& path, int limit, int currentCount) {
    ...
    for (const QFileInfo& info : list) {
        currentCount++;
        if (currentCount >= limit) return limit;
