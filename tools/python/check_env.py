...
功能说明:
    1. 检查构建依赖: 验证 CMake, Ninja/Make, 编译器是否可用。
    2. 检查 Qt 环境: 验证 Qt6 核心组件及翻译工具 (lupdate, lrelease) 是否可用。
    3. 工具链路径管理: 优先从系统环境变量中查找，支持自定义路径变量回退。
...
使用示例:
    ...
    # 仅检查 Qt 开发环境
    python tools/python/check_env.py --qt-only
