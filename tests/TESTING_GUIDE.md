...
在 CI (GitHub Actions) 环境中，测试行为会有所调整以提高稳定性：

*   **跳过网络测试**：检测到 `CI` 环境变量时，依赖真实网络请求的测试（如 `test_component_service` 中的抓取）会被跳过 (`QSKIP`)，以防止因网络波动导致的构建失败。
*   **性能测试警告**：性能基准测试不会因为超时而使构建失败，而是输出警告信息。
