...
int main(int argc, char* argv[]) {
    ...
    // 测试 1: 基本图层映射
    qDebug() << "【测试 1: 基本图层映射】";
    QMap<int, QString> testLayers = {{1, "TopLayer"},
                                     {2, "BottomLayer"},
                                     {3, "TopSilkLayer"},
                                     {4, "BottomSilkLayer"},
                                     {5, "TopPasteMaskLayer"},
    ...
    for (int layerId : testLayerIds) {
        ...
        if (mapper.isSignalLayer(layerId))
            types << "信号层";
        if (mapper.isSilkLayer(layerId))
            types << "丝印层";
        if (mapper.isMaskLayer(layerId))
            types << "阻焊层";
        if (mapper.isPasteLayer(layerId))
            types << "助焊层";
        if (mapper.isMechanicalLayer(layerId))
            types << "机械层";
    ...
    qDebug() << "\n【测试 4: 内层映射】";
    // 测试内层映射
    qDebug() << "  Inner1 (ID=21) ->" << mapper.mapToKiCadLayer(21)
             << mapper.getKiCadLayerName(mapper.mapToKiCadLayer(21));
    qDebug() << "  Inner16 (ID=36) ->" << mapper.mapToKiCadLayer(36)
             << mapper.getKiCadLayerName(mapper.mapToKiCadLayer(36));
    qDebug() << "  Inner32 (ID=52) ->" << mapper.mapToKiCadLayer(52)
             << mapper.getKiCadLayerName(mapper.mapToKiCadLayer(52));
    ...
    qDebug() << "\n【测试 6: 实际封装数据示例】";
    // 模拟实际封装数据
    qDebug() << "  焊盘 1: 位置 (4000, 2980.315) mil -> (" << LayerMapper::milToMm(4000) << ", "
