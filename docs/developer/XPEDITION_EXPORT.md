...
| 能力 | 状态 | 说明 |
| --- | --- | --- |
| 符号导出 | 已实现 | 输出 Xpedition ASCII 符号文本，支持引脚、矩形、折线、多边形、圆和三点圆弧 |
| 封装导出 | 已实现 | 输出 Padstack 和 Cell HKP 文本 |
...
- 封装圆弧、文本、区域和独立孔会写入导出诊断。
- 封装中的 3D 模型引用会写入未关联诊断。
- 符号椭圆、扇形、椭圆弧、路径、Bézier、IEEE 图形、普通文本、文本框和图片会写入未写入诊断。
