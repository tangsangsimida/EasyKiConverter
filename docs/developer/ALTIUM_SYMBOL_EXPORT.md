...
- 符号主体图形：矩形、线段、圆/椭圆、圆弧、多边形、折线、Path、三次 Bézier 曲线和文本。
- 引脚几何：位置、长度、方向、名称、编号、显示状态和部件归属。
- 引脚电气类型：输入、输出、双向、无源、电源、开集电极和开集发射极。
- 引脚装饰：取反圆点、时钟、取反时钟、低电平有效、开集电极、开集发射极、高阻、脉冲、延迟、左移、模拟输入、无逻辑连接和信号方向标志。
...
- 来源 SVG Path 仍会按解析后的点序列导入；调用方可通过 `SymbolBezierIR` 直接保留四控制点 Bézier 曲线。
- `GroupLine`、`FlagLeft` 和 `FlagRight` 使用 Altium 最接近的信号流图形近似表达；如果来源要求精确的 IEEE 专用图形，仍需目标 AD 版本实机确认。
