...
### 新增 (Added)
- **配置自动保存**: 实现了配置项的实时自动保存功能。现在修改任何设置（如输出路径、库名称、导出选项）都会立即写入配置文件，无需手动保存。
- **配置自动补全**: 引入了配置合并与自动补全机制，能够自动修复缺失字段的配置文件（如旧版本升级带来的兼容性问题）。
- **交互优化**: "开始转换"按钮新增了智能状态检查，只有当至少选择了一个导出目标（符号/封装/3D模型）且有待处理元件时才会启用，防止误操作。

### 优化 (Improved)
- **窗口适配**: 将默认启动窗口大小调整为屏幕可用区域的 80%，并将最小窗口尺寸优化为 800x600，完美适配 1366x768 等小屏幕设备。
- **调试模式**: 优化了调试模式的记忆逻辑。现在程序能正确记住用户上次关闭时的调试模式状态，并在启动时恢复。

### 修复 (Fixed)
- 修复了配置重置后 UI 界面未及时刷新的问题。
- 修复了调试模式复选框状态与后端配置不同步的 Bug。
