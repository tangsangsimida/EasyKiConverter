...
当前基线：`src` 359 个文件、74,999 行；`tests` 58 个文件、18,581 行；翻译资源 2 个文件、3,303 行；`tools/python` 13 个文件、6,642 行。项目工具的 `--all` 统计覆盖产品源码、测试和翻译资源，工具目录单独统计后合计 432 个文件、103,525 行。
...
- `ComponentListViewModel.cpp`（1,352 行）：已提取列表状态统计、编号索引、预览更新缓冲和验证队列状态职责，后续继续拆分搜索、选择和批量操作
- `ComponentService.cpp`（1,213 行）：已提取组件数据内存缓存、基础信息字段解析和后台缓存加载职责，后续继续按异步请求状态边界拆分获取与错误处理
...
| 文件 | 总行数 | 代码行 | 问题分析 |
|------|--------|--------|---------|
| `src/ui/qml/components/ComponentListCard.qml` | 371 | -- | 卡片状态协调、过滤触发和弹窗定位仍在一个文件，预览弹窗、工具栏和列表视图已提取 |
