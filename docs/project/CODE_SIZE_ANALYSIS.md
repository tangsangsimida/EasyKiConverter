...
当前基线：`src` 338 个文件、74,389 行；`tests` 58 个文件、18,527 行；翻译资源 2 个文件、3,303 行；`tools/python` 13 个文件、6,642 行。项目工具的 `--all` 统计覆盖产品源码、测试和翻译资源，工具目录单独统计后合计 411 个文件、102,861 行。
...
- `ComponentListViewModel.cpp`（1,372 行）：已提取列表状态统计和编号索引职责，后续继续拆分搜索、选择和批量操作
