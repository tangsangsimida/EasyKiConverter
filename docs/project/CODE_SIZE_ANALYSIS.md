...
当前基线：`src` 336 个文件、74,221 行；`tests` 58 个文件、18,500 行；翻译资源 2 个文件、3,303 行；`tools/python` 13 个文件、6,642 行。项目工具的 `--all` 统计覆盖产品源码、测试和翻译资源，工具目录单独统计后合计 409 个文件、102,666 行。
...
| `src/ui/viewmodels/ComponentListViewModel.cpp` | 1,325 | 1,022 | 109 | ViewModel 职责过多：列表管理+搜索+选择+批量操作 |
| `src/services/ComponentService.cpp` | 1,332 | -- | -- | 数据获取+解析+缓存+错误处理全在一个类 |
...
**需要独立拆分的**：
- `ComponentCacheService.cpp`（1,485 行）：已提取缓存文件布局、目录迁移、元数据存储、数据校验、tombstone 和 L1 存储职责，后续继续按锁边界拆分读写协调逻辑
- `ComponentListViewModel.cpp`（1,325 行）：已提取列表状态统计职责，后续继续拆分搜索、选择和批量操作
- `ComponentService.cpp`（1,332 行）：已提取组件数据内存缓存职责，后续继续拆分获取、解析和错误处理
