...
当前基线：`src` 359 个文件、74,915 行；`tests` 58 个文件、18,588 行；翻译资源 2 个文件、3,303 行；`tools/python` 13 个文件、6,642 行。项目工具的 `--all` 统计覆盖产品源码、测试和翻译资源，工具目录单独统计后合计 432 个文件、103,448 行。
...
| `src/core/altium/writers/AltiumSchLibWriter.cpp` | 1,945 | SchLib 记录写入；来源顺序调度、字体表和图片 Storage 编码已提取 |
...
**可接受但需关注的**（导出器和写入器）：
- `AltiumSchLibWriter.cpp`（1,945 行）：二进制格式写入天然较长，来源顺序调度、字体表和图片 Storage 编码已提取，后续可按记录类型继续拆分
