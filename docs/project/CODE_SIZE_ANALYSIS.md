...
当前基线：`src` 443 个文件、77,690 行；`tests` 58 个文件、18,725 行；翻译资源 2 个文件、3,303 行；`tools/python` 13 个文件、6,640 行。项目工具的 `--all` 统计覆盖产品源码、测试和翻译资源，工具目录单独统计后合计 516 个文件、106,358 行。
...
| `src/ui/viewmodels/ComponentListViewModel.cpp` | 734 | -- | -- | ViewModel 职责过多：列表管理+搜索+选择；批量导入、剪贴板处理、列表数据回调、预览更新缓冲、列表项更新缓冲、验证队列状态、验证错误分类、定时器初始化和服务信号连接已提取 |
...
- `ComponentListViewModel.cpp`（734 行）：已提取列表状态统计、编号索引、批量导入、剪贴板处理、组件服务数据回调、预览更新缓冲、列表项更新缓冲、验证队列状态、验证错误分类、定时器初始化、服务信号连接和验证队列协调职责，并移除失效的预览图编码线程池，后续继续拆分搜索和选择
