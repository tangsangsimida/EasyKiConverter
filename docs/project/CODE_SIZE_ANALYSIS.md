...
当前基线：`src` 350 个文件、74,705 行；`tests` 58 个文件、18,527 行；翻译资源 2 个文件、3,303 行；`tools/python` 13 个文件、6,642 行。项目工具的 `--all` 统计覆盖产品源码、测试和翻译资源，工具目录单独统计后合计 423 个文件、103,177 行。
...
| 文件 | 总行数 | 代码行 | 注释行 | 问题分析 |
|------|--------|--------|--------|---------|
| `src/services/ComponentCacheService.cpp` | 1,390 | -- | -- | 缓存读写/过期/迁移逻辑仍集中，已通过辅助职责拆分逐步收敛 |
...
**需要独立拆分的**：
- `ComponentCacheService.cpp`（1,390 行）：已提取缓存文件布局、目录迁移、元数据存储、组件数据读取、二进制文件读取、数据校验、tombstone 和 L1 存储职责，后续继续按锁边界拆分读写协调逻辑
