...
当前基线：`src` 383 个文件、75,672 行；`tests` 58 个文件、18,632 行；翻译资源 2 个文件、3,303 行；`tools/python` 13 个文件、6,640 行。项目工具的 `--all` 统计覆盖产品源码、测试和翻译资源，工具目录单独统计后合计 456 个文件、104,247 行。
...
| 文件 | 总行数 | 代码行 | 注释行 | 问题分析 |
|------|--------|--------|--------|---------|
| `src/services/ComponentCacheService.cpp` | 1,213 | -- | -- | 缓存目录、生命周期、预览图、数据手册和管理流程仍集中，元数据、3D 文件、CAD 数据和预览图写入已提取 |
...
**需要独立拆分的**：
- `ComponentCacheService.cpp`（1,213 行）：已提取缓存文件布局、目录迁移、元数据存储、组件数据读取、二进制文件读取、CAD 数据写入、预览图写入、写入代次策略、数据校验、tombstone、L1 存储、元数据写入协调和 3D 文件 I/O 职责，后续继续按锁边界拆分管理流程
