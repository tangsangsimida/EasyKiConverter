...
当前基线：`src` 478 个文件、78,548 行；`tests` 58 个文件、18,726 行；翻译资源 2 个文件、3,303 行；`tools/python` 13 个文件、6,640 行。项目工具的 `--all` 统计覆盖产品源码、测试和翻译资源，工具目录单独统计后合计 551 个文件、107,217 行。
...
| `src/core/altium/ExporterAltiumSymbol.cpp` | 842 | SchLib 符号转换；引脚、曲线图元、几何归一化、部件 ID/方向/颜色/线型和坐标量化共享规则已提取 |
...
- `ExporterAltiumSymbol.cpp`（842 行）：已提取符号引脚、曲线图元、几何归一化、部件 ID/方向/颜色/线型和坐标量化共享规则，主文件继续负责符号记录编排、矩形/文本/图片转换和模型关联
