...
网络失败时会先应用有效的本地 Release 缓存，再将状态标记为“检查失败”，因此用户可以继续看到已缓存的更新信息，同时版本对话框仍会显示本次检查失败。失败状态提供“查看更新”入口：有缓存时打开缓存的 Release 页面，否则打开官方 `releases/latest` 页面。缓存和忽略记录在发现更高版本后自动清理。
